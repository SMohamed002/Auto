...
if (!empty($item_id) && is_numeric($item_id) && $item_id > 0) {
    $exist = Info::getQuery("select id from wishlist where deleted = 'N' and client_id = ? and item_id = ?", ["i_" . $_SESSION['client_id'], "i_" . $item_id]);
    if(!empty($exist) && is_countable($exist) && count($exist) > 0 && is_numeric($exist[0]['id']) && $exist[0]['id'] > 0){
        Info::executeQuery("update wishlist set deleted = 'Y',updated_at = sysdate() where deleted = 'N' and id = ?", ["i_".$exist[0]['id']]);
    }else{
        echo '<script>
                Swal.fire({
                    position: "top-end",

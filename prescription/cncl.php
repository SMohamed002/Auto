...
if(empty($id) || !is_numeric($id) || $id <= 0) {
    ...
    $deleted = Info::executeQuery("update prescriptions set is_ordered = 'N',updated_at = sysdate() where is_ordered = 'Y' and is_done = 'N' and id = ?;",["i_".$id]);
    if($deleted){
        echo '<script>
                Swal.fire({
                    position: "top-end",
        ...
        echo '<script>
                Swal.fire({
                    position: "top-end",
                    icon: "error",
                    title: "An error occurred while cancelling order.",
                    toast: true,
                    timerProgressBar: true,
                    showConfirmButton: false,
                    showCloseButton: true,
